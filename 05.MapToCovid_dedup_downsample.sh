directory=/scratch/2020-09-25/bio-zhaoy1/Ribozero/3.star_mapping_to_covid/dedup
outdir=/scratch/2020-09-25/bio-zhaoy1/Ribozero/3.star_mapping_to_covid/dedup/DownSampling
mkdir -p $outdir
cd $directory
DIR=/scratch/2020-09-25/bio-zhaoy1/90.scripts/star_jobs
samples=`ls VC12*.no_dup.bam`
for sample in $samples
do
samplename=`basename $sample .no_dup.bam`
#fq1=$sample
#fq2=$samplename\_Unmapped.out.mate2.fastq.gz
cd $DIR
config=$samplename\_ribozeroToVirus_dedup_downsample.lsf
touch $config
...
echo "#BSUB -o "$samplename"_riboToVirus_dedup_ds.out" >> $config
echo "#BSUB -e "$samplename"_riboToVirus_dedup_ds.err" >> $config
...
!
#echo "samtools view -@ 40 -s 0.214 -h -b \$base.no_dup.bam -o \$outdir/\$base.no_dup_ds.bam" >> $config
#echo "samtools view -@ 40 -s 0.219 -h -b \$base.no_dup.bam -o \$outdir/\$base.no_dup_ds.bam" >> $config
#echo "samtools view -@ 40 -s 0.911 -h -b \$base.no_dup.bam -o \$outdir/\$base.no_dup_ds.bam" >> $config
echo "samtools view -@ 40 -s 0.860 -h -b \$base.no_dup.bam -o \$outdir/\$base.no_dup_ds.bam" >> $config
...
echo "#extract cigar containing jumps" >> $config
echo "samtools view \$base.no_dup_ds.bam | awk 'BEGIN {{ OFS=\"\t\"; }} {{ if (\$6 ~ /N/) print \$2,\$4,\$6; }}' > \$jumpdir/\$base.cigar_jumps.txt" >> $config

echo "#counting jump sites" >> $config
echo "cat \$jumpdir/\$base.cigar_jumps.txt |python /scratch/2020-09-25/bio-zhaoy1/90.scripts/convert-cigars-singlestrand.py| sort -k2,3n | uniq -c | awk '{{ OFS=\"\t\"; }} {{ print \$3,\$4,\$2,\$1; }}' > \$jumpdir/\$base.jumps.txt" >> $config

echo "cd "$outdir >> $config
echo "coverage=coverage-bystrand" >> $config
echo "mkdir -p \$coverage" >> $config

echo "#calculate coverage" >> $config
echo "bedtools genomecov -ibam  \$base.no_dup_ds.bam -dz -split > \$coverage/\$base.dedup.mapped.coverage.txt" >> $config
echo "#pileup reads" >> $config
echo "genomefa=/work/bio-zhaoy1/03.Reference/COVID19/COVID19.fasta" >> $config
echo "samtools mpileup -f \$genomefa \$base.no_dup_ds.bam > \$coverage/\$base.dedup.sort.mapped.pileup.txt" >> $config

echo "cd \$outdir" >> $config
echo "bedtools genomecov -ibam \$base.no_dup_ds.bam -dz -split -du -strand + > \$coverage/\$base.dedup.mapped.coveragebyStrand.txt" >> $config
echo "bedtools genomecov -ibam \$base.no_dup_ds.bam -dz -split -du -strand - > \$coverage/\$base.dedup.mapped.coveragebyNegativeStrand.txt" >> $config
